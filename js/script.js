...
const setActiveTestimonial = (newIndex) => {
    if (newIndex === activeTestimonial) return;
...
const showPrevTestimonial = () => {
    const prevIndex = (activeTestimonial - 1 + testimonialCards.length) % testimonialCards.length;
    setActiveTestimonial(prevIndex);
};

const restartTestimonialTimer = () => {
    clearInterval(testimonialTimer);
    testimonialTimer = setInterval(showNextTestimonial, 5000);
};
...
if (prevButton && nextButton && testimonialCards.length) {
    ...
    testimonialDots.forEach((dot, idx) => {
        dot.addEventListener('click', () => {
            const direction = idx > activeTestimonial ? 'next' : 'prev';
            setActiveTestimonial(idx, direction);
            restartTestimonialTimer();
        });
    });
